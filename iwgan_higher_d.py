...
def sample_data(data, batch_size, weighted=weighted):
    assert data.shape[1] == x_dim, 'data shape != x_dim'
    ...
    if generated.shape[1] == 2:
        ...
        for i in range(grid_gran):
            for j in range(grid_gran):
                grid_x_normed = np.reshape((grid_x[i] - data_raw_mean[0]) / data_raw_std[0], [-1, 1])
                grid_y_normed = np.reshape((grid_y[j] - data_raw_mean[0]) / data_raw_std[0], [-1, 1])
                x_in = np.concatenate((grid_x_normed, grid_y_normed), axis=1)
                vals_on_grid[i][j] = run_discrim(x_in)
...
if weighted:
    x_unnormed = (x * data_raw_std + data_raw_mean)[:, :1]
